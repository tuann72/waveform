...
import { COLOR_PALETTE, pickColorOptions } from "@/lib/colorPalette"
...
export function ColorformClueView() {
  ...
  const colorOptions = useStorage((s) =>
    s ? (s.colorOptions as Record<string, number[][]>) : ({} as Record<string, number[][]>),
  ) ?? {}
  ...
  const [clues, setClues] = useState<string[]>(() => Array(totalRounds).fill(""))
  ...
  useEffect(() => {
    if (!colorOptions[mp.playerId]) {
      const opts = Array.from({ length: totalRounds }, () => pickColorOptions())
  ...
  useEffect(() => {
    ...
    const finalColors = Array.from({ length: totalRounds }, (_, i) => selectedColors[i] ?? 0)
    const finalClues = Array.from({ length: totalRounds }, (_, i) => clues[i] ?? "")
      ...
      <div className="w-full max-w-sm flex flex-col gap-5">
        ...
        {!myCluesSubmitted ? (
          <>
              ...
              <div className="flex justify-center gap-4">
                {currentOptions.map((colorIdx) => (
                  <button
                    ...
                    className={cn(
                      "w-20 h-20 rounded-2xl transition-all focus:outline-none",
                      currentSelectedIndex === colorIdx
                        ? "scale-110 shadow-xl"
                        : "opacity-80 hover:opacity-100 hover:scale-105",
                    )}
                    style={{
                      background: COLOR_PALETTE[colorIdx],
                      boxShadow:
                        currentSelectedIndex === colorIdx
                          ? `0 0 0 3px white, 0 0 0 5px ${COLOR_PALETTE[colorIdx]}`
                          : undefined,
                    }}
                    aria-label={COLOR_PALETTE[colorIdx]}
                  />
                ))}
              </div>
            </div>
            ...
            {currentSelectedIndex !== null ? (
              <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/40 border">
                <div
                  ...
                  style={{ background: COLOR_PALETTE[currentSelectedIndex] }}
                />
                <div>
                  <p className="text-xs text-muted-foreground">Your target color</p>
                  <p className="text-sm font-mono text-foreground">
                    {COLOR_PALETTE[currentSelectedIndex]}
                  </p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/20 border border-dashed">
                ...
                <p className="text-sm text-muted-foreground">Select a color above</p>
              </div>
            )}
            ...
            <div className="flex flex-col gap-2">
              <Input
                key={currentRound}
                placeholder="Describe this color in one word…"
              ...
              <div className="flex gap-2">
                ...
                {currentRound < totalRounds - 1 ? (
                  <Button
                    className="flex-1"
                    disabled={currentSelectedIndex === null || !clues[currentRound]?.trim()}
                    onClick={() => setCurrentRound((n) => n + 1)}
                  >
                    Next →
                  </Button>
                ) : (
                  <Button className="flex-1" disabled={!canSubmit} onClick={handleSubmitAll}>
                    Submit All Clues
                  </Button>
                )}
              </div>
        ...
        <div className="flex flex-col gap-1.5">
          {players.map(([id, info]) => {
            ...
            const fullyDone = saved.length >= totalRounds && saved.every((c) => c.trim())
            ...
            return (
              <div key={id} className="flex items-center justify-between text-sm">
                ...
                <Badge variant={fullyDone ? "default" : "outline"}>
                  {fullyDone ? "Ready" : partial ? "Partial" : <><span>Picking</span><Ellipsis /></>}
                </Badge>
              </div>
            )
          })}
